...
import (
	...
	itemservicesvr "assignment_crossnokaye/cod/itemservice/gen/grpc/item_service/server"
...
func handleGRPCServer(ctx context.Context, u *url.URL, itemServiceEndpoints *itemservice.Endpoints, wg *sync.WaitGroup, errc chan error, logger *log.Logger, debug bool) {
	...
	// responses.
	var (
		itemServiceServer *itemservicesvr.Server
	)
	{
		itemServiceServer = itemservicesvr.New(itemServiceEndpoints, nil)
	}
	...
	item_servicepb.RegisterItemServiceServer(srv, itemServiceServer)
