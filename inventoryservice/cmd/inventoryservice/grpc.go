...
import (
	...
	inventoryservicesvr "assignment_crossnokaye/cod/inventoryservice/gen/grpc/inventoryservice/server"
...
func handleGRPCServer(ctx context.Context, u *url.URL, inventoryserviceEndpoints *inventoryservice.Endpoints, wg *sync.WaitGroup, errc chan error, logger *log.Logger, debug bool) {
	...
	// responses.
	var (
		inventoryserviceServer *inventoryservicesvr.Server
	)
	{
		inventoryserviceServer = inventoryservicesvr.New(inventoryserviceEndpoints, nil)
	}
	...
	inventoryservicepb.RegisterInventoryserviceServer(srv, inventoryserviceServer)
